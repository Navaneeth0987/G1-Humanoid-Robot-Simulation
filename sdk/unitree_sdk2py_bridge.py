...
class ElasticBand:
    ...
    # Only support vertical axis
        if self.point is None:
            self.point = np.array([x[0], x[1], 2.4])

        # Follow robot in X and Y
        self.point[0] = x[0]
        self.point[1] = x[1]

        vertical_error = self.point[2] - x[2]
        vertical_velocity = dx[2]

        fz = self.stiffness * vertical_error - self.damping * vertical_velocity
        fz = np.clip(fz, -500, 500)
